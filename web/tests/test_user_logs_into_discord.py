...
class DiscordLoginTestCase(LiveServerTestCase):
    ...
    def test_user_gives_discord_auth_for_login(self):
    ...
    def test_user_denies_discord_auth_for_login(self):
        wait = WebDriverWait(self.driver, 10)
        ...
        discord_oauth_page = DiscordOauthPage(self.driver)
        wait.until(lambda driver: discord_oauth_page.find_cancel_button())
        discord_oauth_page.find_cancel_button().click()
        ...
        # Without an authorization code, user is redirected back to the login page
        wait.until(lambda driver: driver.current_url == (self.live_server_url + "/"))
